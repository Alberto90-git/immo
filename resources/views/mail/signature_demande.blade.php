...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-sign {
    ...
    font-size: 15px; font-weight: bold; text-align: center; max-width: 280px;
  }
  .info-box {
    background: #f8f9ff; border-left: 4px solid #696cff;
    padding: 14px 16px; border-radius: 4px; margin: 16px 0;
    font-size: 13px; color: #475569; line-height: 1.7;
  }
  .warning { font-size: 12px; color: #94a3b8; margin-top: 16px; line-height: 1.6; word-break: break-all; }
...
<div class="wrapper">
  <div class="header">
    <h2>{{ __('sig.mail_subject') }}</h2>
    <p>✍️ {{ __('sig.sign_page_title') }}</p>
  </div>
  <div class="body">
    <p>{{ __('mail.greeting', ['name' => $nom]) }}</p>
    <p>{{ __('sig.sign_intro') }}</p>

    <div class="info-box">
      <strong>{{ __('sig.cert_document') }} :</strong> {{ $titre }}<br>
      @if(isset($expire))
      <strong>{{ __('sig.sign_expires', ['date' => '']) }}</strong> {{ $expire }}
      @endif
    </div>

    <p>{{ __('sig.sign_legal') }}</p>

    <a href="{{ $lien }}" class="btn-sign">✍️ {{ __('sig.sign_btn') }}</a>
