...
<h1 style="padding: 10px;"><u>Fiche de paie de {{ $element2['proprio_nom'] }}  {{ $element2['proprio_prenom'] }} du {{ Carbon\Carbon::parse($element2['date_debut'])->format('d/m/Y') }} au {{ Carbon\Carbon::parse($element2['date_fin'])->format('d/m/Y') }} </u></h1>
   ...
   @if(isset($element2['donnees'] ))
      @foreach($element2['donnees'] as $items)
        <tr>
          ...
          <td>{{ number_format( ( $items->montant * (100 - $element2['pourcentage']) ) / 100,"0",",",".") }} XOF</td>
        </tr>
      @endforeach
    @endif
    <tr>
